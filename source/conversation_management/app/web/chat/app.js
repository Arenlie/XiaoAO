...
function statusText(status){return ({STREAMING:"生成中",COMPLETED:"完成",FAILED:"失败",STOPPED:"已停止",WAITING_INPUT:"等待补充",WAITING_SELECTION:"等待选择",WAITING_CONFIRMATION:"等待分析方式选择",CANCELLED:"已取消",SKIPPED:"已跳过",TIMEOUT:"超时",PENDING:"等待中",STARTED:"已开始",QUEUED:"等待处理",PREPARING:"准备中",PLANNING:"分析中",EXECUTING:"执行中",SUCCESS:"已完成",EXPIRED:"已过期",SUPERSEDED:"已由新问题接续"})[status]||"状态更新";}

const PROGRESS_NAMES={"Asset MCP：提前检索实体": "资产查询：提前检索设备与区域", "Asset MCP：实体检索": "资产查询：设备、测点与区域检索", "ReAct 决策": "查询与分析决策", "ReAct 执行": "执行查询与分析", "主控大模型：ReAct 下一步决策": "主控大模型：下一步处理决策", "PHM 空间树查询": "区域层级查询", "PHM 空间下级查询": "下级区域查询", "PHM 波形查询": "振动波形查询", "PHM 振动标量特征提取": "振动特征指标提取", "PHM 转速/1X 特征提取": "转速及一倍转频特征提取", "PHM 诊断模型准入状态": "诊断服务可用性检查", "设备结构 RAG 状态": "设备结构资料接入状态", "Embedding 模型：生成语义向量": "语义检索准备", "Reranker 模型：候选结果精排": "候选结果相关性排序", "Asset 语义大模型：解析资产查询语义": "资产查询意图解析", "报警 SQL 规划大模型：调用大模型生成 SQL": "报警查询条件生成", "Diagnosis 大模型：调用诊断大模型": "综合诊断分析", "读取本轮所需的注册表、历史上下文和运行状态": "读取相关对话和已确认的查询对象。", "提取目标、范围、证据、操作、时间和资产语义，并判断是否复用可选 Recipe": "明确查询目标、对象范围和时间要求，确定适用的处理流程。", "父步骤墙钟时间；子步骤可能重叠，不应相加。候选就绪可提前进入选择": "部分步骤同时进行；候选对象就绪后及时返回确认。各步骤耗时不应直接相加。", "独立提取本轮根对象，与主控业务路由并行": "识别本轮查询对象，与问题理解和处理方案分析同步进行。", "按轻量模型提取的真实查询范围检索；仅数据库候选可用于选择": "根据已识别的范围提前检索，返回平台中实际登记的候选对象。", "提取业务目标和结构化语义；可与轻量对象识别及实体检索重叠": "明确查询要求和处理方式，与查询对象识别同步进行。", "调用 Asset MCP 完成语义理解、Embedding、PostgreSQL 候选召回、Reranker 精排和实体决策": "根据名称、编号和上下文检索设备、测点与区域，对候选结果进行相关性排序和身份确认。", "依据目标、证据契约和当前观察选择一个最有价值的下一步": "根据当前目标和已有结果，确定下一步查询或分析内容。", "执行当前选中的 MCP、智能体或稳定 Recipe 步骤": "按当前方案执行数据查询、特征计算或诊断分析。", "业务完成后检索相关资料；有独立时间预算，失败不影响已取得的业务事实": "检索相关企业资料和历史案例，为已有查询结果补充参考依据。", "实时输出模型提供的推理和正式回答": "综合已有信息生成分析说明和正式回答，内容陆续显示。"};
const PROGRESS_TERMS={"PHM Asset MCP": "资产查询服务", "PHM Data MCP": "设备数据服务", "PHM Feature MCP": "特征分析服务", "PHM Diagnosis MCP": "专业诊断服务", "Asset MCP": "资产查询服务", "Data MCP": "设备数据服务", "Feature MCP": "特征分析服务", "Diagnosis MCP": "专业诊断服务", "Embedding 模型": "语义匹配", "Reranker 模型": "相关性排序", "Asset 语义大模型": "资产查询分析", "Diagnosis 大模型": "诊断分析", "报警 SQL 规划大模型": "报警查询分析", "MongoDB 数据库": "监测数据查询", "MySQL 报警数据库": "报警记录查询", "PostgreSQL": "数据查询", "Embedding": "语义匹配", "Reranker": "相关性排序", "Recipe": "处理流程", "ReAct": "查询与分析", "RAG": "资料检索", "MCP": "服务", "SQL": "查询条件", "LLM": "大模型", "OCR": "图中文字识别"};
const PROGRESS_TOOL_NAMES={"resolve_entity": "设备、测点与区域检索", "query_equipment_info": "设备资产详情查询", "query_space_tree": "区域层级查询", "query_space_children": "下级区域查询", "query_devices": "区域设备查询", "query_scope_collection": "区域下属实体集合查询", "query_points": "设备测点查询", "query_active_sensor_faults": "传感器当前故障查询", "query_sensor_fault_history": "传感器历史故障查询", "query_offline_sensors": "离线传感器查询", "query_sensor_monitoring_status": "传感器监测与在线状态查询", "query_sensor_status_overview": "传感器状态概览查询", "query_monitored_sensor_points": "传感器监测测点查询", "get_sensor_fault_evidence": "传感器故障证据详情", "get_waveform": "振动波形查询", "get_feature_trend": "特征趋势查询", "get_temperature_trend": "温度趋势查询", "check_data_availability": "数据可用性检查", "get_data_snapshot": "数据快照查询", "get_device_data": "整设备全部测点数据查询", "query_health_score": "健康度查询", "query_alarm_records": "综合报警查询", "diagnose_point": "单测点综合诊断", "diagnose_device": "整设备全部测点综合诊断", "analyze_chart": "单图谱诊断分析", "get_model_admission": "诊断服务可用性检查", "extract_vibration_features": "振动特征指标提取", "extract_rotational_speed_feature": "转速及一倍转频特征提取"};
function progressText(value){let text=String(value??"");const root=text.match(/^执行\s+([a-z][a-z0-9_.]*)$/);if(root)return PROGRESS_TOOL_NAMES[root[1]]||"执行本项查询或分析";if(/(?:中执行|工具)\s*[A-Za-z_][A-Za-z0-9_.]*/.test(text))return "执行本项查询或分析。";text=PROGRESS_NAMES[text]||text;for(const [a,b] of Object.entries(PROGRESS_TERMS))text=text.split(a).join(b);return text.replace(/\bPHM\s*/g,"").replace(/(?<=：)[A-Za-z_][A-Za-z0-9_]*$/g,"执行本项处理");}
function progressActor(value){const text=String(value||"");if(text.includes("supervisor"))return "主控分析";if(/asset|entity/.test(text))return "资产查询";if(/diagnosis/.test(text))return "诊断分析";if(/feature|rpm/.test(text))return "特征计算";if(/knowledge|dify/.test(text))return "企业资料检索";if(/file|attachment/.test(text))return "附件处理";if(/data|alarm|health/.test(text))return "设备数据查询";return "查询与分析服务";}
const PROGRESS_FIELDS={equip_name:"设备名称",equipment_name:"设备名称",equip_no:"设备编号",point_name:"测点名称",point_no:"测点编号",space_name:"区域名称",display_name:"名称",objective:"本次目标",reason:"处理说明",public_message:"结果说明",display_message:"处理说明",question:"待确认内容",requested_count:"查询数量",success_count:"完成数量",failure_count:"未完成数量",completed_count:"已完成数量",supported_count:"可用结果数量",candidate_count:"候选数量",record_count:"记录数量",duration_ms:"耗时（毫秒）",total_elapsed_ms:"查询耗时（毫秒）",first_reasoning_ms:"本步骤等待分析说明的时间（毫秒）",first_final_ms:"本步骤等待正文的时间（毫秒）",output_chars:"正文字符数",dataTime:"数据时间",data_time:"数据时间",start_time:"起始时间",end_time:"结束时间",status:"处理状态",monitoring_status:"监测状态",cache_hit:"复用已有结果",output_limited:"明细仅展示部分内容",registry_complete:"监测清单已确认完整",required_level:"定位要求",required_entity_level:"定位要求",entity_status:"匹配情况",requested_trend_hours:"查询时间范围（小时）",effective_trend_hours:"实际取得的时间范围（小时）",reasoning_available:"已提供分析说明",optional:"辅助查询步骤",affects_task:"影响本轮处理结果",finish_reason:"结束说明"};
const PROGRESS_VALUES={equipment:"设备",point:"测点",space:"区域",area:"区域",none:"无需定位",UNIQUE:"唯一匹配",MULTIPLE:"多个候选",NO_LOOKUP:"无需重新检索",NOT_FOUND:"未找到",ONLINE:"在线",OFFLINE:"离线",SUSPENDED:"暂停诊断",UNKNOWN:"暂时无法确认",NOT_MONITORED:"未纳入监测",worker_finished:"本轮处理已结束",superseded_or_stopped:"处理已转入其他步骤或已停止"};
function progressDetails(value){
  if(!value||typeof value!=="object")return {};
  const out={};
  for(const [key,raw] of Object.entries(value)){
    if(!PROGRESS_FIELDS[key]||raw==null)continue;
    let text=typeof raw==="boolean"?(raw?"是":"否"):raw;
    if(typeof text==="object")continue;
    if(["status","monitoring_status"].includes(key))text=PROGRESS_VALUES[text]||statusText(text);
    else if(PROGRESS_VALUES[text])text=PROGRESS_VALUES[text];
    else if(!["equip_no","point_no","equip_name","equipment_name","point_name","space_name","display_name"].includes(key)&&typeof text==="string"){
      text=progressText(text);
      if(/\b[a-z]+_[a-z_]+\b|\b(?:builtin|workflow|mcp)\.|https?:\/\//i.test(text))continue;
    }
    out[PROGRESS_FIELDS[key]]=text;
  }
  return out;
}
...
function eventTitle(event){const labels={
  ...
  "tool.started":"查询服务调用开始","tool.retrying":"查询服务正在重试","tool.completed":"查询服务调用完成","tool.failed":"查询服务调用失败",
  "performance.span.started":"执行步骤开始","performance.span.completed":"执行步骤完成","performance.span.failed":"执行步骤失败","performance.mcp.details":"查询与分析历史明细",
...
};return labels[event]||"处理进度更新";}
function formatDurationMs(value){const ms=Number(value);if(!Number.isFinite(ms))return "—";if(ms<1)return `${ms.toFixed(3)} 毫秒`;if(ms<1000)return `${ms.toFixed(ms<10?2:1)} 毫秒`;return `${(ms/1000).toFixed(ms<10000?3:2)} 秒`;}
function perfCategoryLabel(value){return ({llm:"大模型",mcp:"查询服务",mcp_tool:"查询工具",database:"数据库",embedding:"语义匹配",reranker:"相关性排序",algorithm:"算法",decode:"解码",planning:"规划",workflow:"业务流程",content:"内容处理",context:"上下文",internal:"流程处理",retrieval:"资料检索",orchestration:"流程处理"})[value]||"流程处理";}
...
function renderMcpInternalTrace(trace){
  ...
  if(!spans.length)return `<div class="perf-muted">该查询服务未提供分步计时；此处显示整体处理耗时。</div>`;
  return `<div class="perf-mcp-tree">${spans.map((item,i)=>`<div class="perf-mcp-row" style="--depth:${item.parent_span_id?1:0}"><span class="perf-status ${item.status==='FAILED'?'failed':'ok'}">${item.status==='FAILED'?'×':'✓'}</span><div><strong>${escapeHtml(progressText(item.name||`处理步骤 ${i+1}`))}</strong><small>${escapeHtml(progressText(item.description||""))}</small></div><b>${escapeHtml(formatDurationMs(item.duration_ms))}</b><em>${escapeHtml(perfCategoryLabel(item.category))}</em></div>`).join("")}</div>`;
}
...
function renderExecutionEvent(row,index,allEvents){
  ...
  const detail=progressDetails(payload);for(const key of ["content","channel","trace","execution_trace","name_cn","description_cn","category","metrics","internal_monitor_enabled","sequence_no","span_id","parent_span_id","task_id","message_id","graph_mode","created_at","actor_type","actor_id","duration_ms"])delete detail[key];
  const title=progressText(payload.name_cn||eventTitle(row.event_type));
  ...
  const metrics=payload.metrics&&Object.keys(progressDetails(payload.metrics)).length?`<details class="execution-data"><summary><span>用时说明</span><code>${escapeHtml(JSON.stringify(progressDetails(payload.metrics)).slice(0,160))}</code></summary><pre>${escapeHtml(pretty(progressDetails(payload.metrics)))}</pre></details>`:"";
  const traceHtml=trace?`<details class="perf-mcp-detail" open><summary><strong>查询与分析明细</strong><span>${Number(trace.span_count||trace.spans?.length||0)} 步 · ${escapeHtml(formatDurationMs(trace.total_duration_ms))}</span></summary>${renderMcpInternalTrace(trace)}</details>`:(row.event_type==="tool.completed"&&payload.internal_monitor_enabled===false?`<div class="perf-muted compact-note">该查询服务的详细计时未开启，本流程只显示整体调用耗时。</div>`:"");
  return `<div class="${classes.join(" ")}"><div class="execution-index">${running?'●':index+1}</div><div class="execution-step-body"><div class="execution-step-head"><div><strong>${escapeHtml(title)}</strong>${category?`<em class="execution-category">${escapeHtml(category)}</em>`:""}</div><span>${timeText}</span></div>${desc?`<div class="execution-decision">${escapeHtml(progressText(desc))}</div>`:""}<div class="execution-field"><span>执行者</span><strong>${escapeHtml(progressActor(actor))}</strong></div>${status?`<div class="execution-field"><span>状态</span><strong>${escapeHtml(statusText(status))}</strong></div>`:""}${metrics}${traceHtml}${Object.keys(detail).length?`<details class="execution-data"><summary><span>查询详情</span><code>${escapeHtml(JSON.stringify(detail).slice(0,160))}</code></summary><pre>${escapeHtml(pretty(detail))}</pre></details>`:""}${row.error_message?`<div class="execution-decision">${escapeHtml(progressText(payload.public_message||payload.display_message||"本项处理未能完成，详见查询结果说明。"))}</div>`:""}</div></div>`;
}
...
function renderProcessPanel(message){
  ...
  const cards=summary.detailed?`<div class="perf-cards"><div><span>排队等待</span><strong>${formatDurationMs(summary.queue)}</strong></div><div><span>智能分析累计用时</span><strong>${formatDurationMs(summary.llm)}</strong></div><div><span>查询与分析累计用时</span><strong>${formatDurationMs(summary.mcp)}</strong></div><div><span>数据读写累计用时</span><strong>${formatDurationMs(summary.database)}</strong></div></div>`:"";
  const hint=!summary.detailed&&summary.toolCount?`<div class="perf-off-note">详细时间监测未开启；执行流程接口保持不变，仍显示 ${summary.toolCount} 次 查询与分析整体耗时。</div>`:"";
...
function refreshLiveExecutionTimers(){document.querySelectorAll("[data-execution-running-start]").forEach(target=>{const started=Number(target.dataset.executionRunningStart||Date.now());target.textContent=`已运行 ${Math.max(0,(Date.now()-started)/1000).toFixed(1)} 秒`;});}
...
function renderDiagnosisChoice(message){
  const c=state.diagnosisConfirmation;
  if(!c||String(c.message_id)!==String(message.id))return "";
  const expired=Date.parse(c.expires_at)<=Date.now();
  return `<section class="diagnosis-choice"><p>${expired?"本次确认已过期，未启动详细诊断。":"在下方回复“快速分析”或“详细诊断”，也可以直接提出其他问题。"}</p></section>`;
}
...
function acceptDiagnosisPause(data){if(state.settledDiagnosisIds.has(data.confirmation_id))return;state.diagnosisConfirmation={...data};const completed=data.status==="COMPLETED"||data.stream_continues===false;const m=messageById(data.message_id);if(m){m.status=completed?"COMPLETED":"WAITING_CONFIRMATION";if(data.question)m.content=data.question;}state.isGenerating=false;if(!completed)state.streamAbort?.abort();updateGeneratingUi();renderMessages();}
...
function renderTrace(){const box=$("traceTimeline");$("traceTaskId").textContent=state.currentTaskId?"本轮处理记录":"尚无任务";if(!state.traceEvents.length){box.innerHTML='<div class="trace-empty">发送问题后，这里会实时显示与消息卡片一致的执行流程和每一步计时。</div>';return;}const events=state.traceEvents.filter(row=>!isFinalContentEventRow(row));const rows=unifiedExecutionRows(events);box.innerHTML=rows.map((row,index)=>{const p=eventPayload(row);const running=runningExecutionRow(row,events);const duration=row.duration_ms!=null?formatDurationMs(row.duration_ms):(running?"运行中":"");const trace=mcpTraceFromEvent(row);return `<div class="trace-event ${running?'running':''}"><div class="trace-dot"></div><div><strong>${escapeHtml(progressText(p.name_cn||eventTitle(row.event_type)))}</strong><span>${escapeHtml(duration||progressText(p.description_cn)||progressActor(row.actor_id))}</span>${p.description_cn&&duration?`<small>${escapeHtml(progressText(p.description_cn))}</small>`:""}${trace?renderMcpInternalTrace(trace):""}</div></div>`;}).join("");box.scrollTop=box.scrollHeight;}
...
async function sendMessage(prefilled=null){
  ...
  try{
    ...
    const previousChoice=state.diagnosisConfirmation;
    if(previousChoice){state.settledDiagnosisIds.add(previousChoice.confirmation_id);const previousMessage=messageById(previousChoice.message_id);if(previousMessage&&previousMessage.status!=="COMPLETED")previousMessage.status="STOPPED";}
    state.diagnosisConfirmation=null;state.currentTaskId=accepted.task_id;state.currentAssistantId=accepted.assistant_message_id;state.currentConversationId=accepted.conversation_id;state.currentStreamUrl=accepted.agent_stream_url||accepted.stream_url||null;state.streamLastEventId="0-0";state.terminalReceived=false;state.appliedContentEventIds.clear();state.traceEvents=[];
...
function applyFinalContentDelta(eventType,data,eventId="",assistantId=null){
  ...
  if(data.output_kind==="confirmation_prompt"&&!data.generation_id){const current=outputFor(mid);if(!current?.generationId){assistant.content=content;assistant.status=data.status==="COMPLETED"?"COMPLETED":"WAITING_CONFIRMATION";renderMessages();}return true;}
