...
function statusText(status){return ({STREAMING:"生成中",COMPLETED:"完成",FAILED:"失败",STOPPED:"已停止",WAITING_INPUT:"等待补充",WAITING_SELECTION:"等待选择",TIMEOUT:"超时",PENDING:"等待中"})[status]||status||"";}
...
function renderMessages(){
  ...
  list.innerHTML=messages.map((message,index)=>{
    try{
      ...
      const streaming=message.status==="STREAMING"||(state.isGenerating&&String(message.id)===String(state.currentAssistantId));
      ...
      const body=assistant
        ? waitingSelection
          ...
          : `${renderProcessPanel(message)}<section class="final-answer"><div class="final-answer-heading"><strong>最终回答</strong><span>${escapeHtml(statusText(message.status))}</span></div><div class="final-answer-content message-text">${renderMarkdown(content|| (streaming?"正在生成最终回答……":""))}${streaming?'<span class="typing-cursor"></span>':""}</div></section>`
...
async function loadExecutionEventsForMessages(messages){const assistants=normalizeMessageRows(messages).filter(item=>normalizeMessageRole(item.role)==="ASSISTANT");await Promise.all(assistants.map(async message=>{try{const result=await apiRequest(`/chat/v1/messages/${message.id}/execution-events`);state.messageEvents.set(String(message.id),result.data||[]);}catch(_){state.messageEvents.set(String(message.id),[]);}}));}
...
async function sendMessage(prefilled=null){
  ...
  try{
    ...
    const accepted=result.data;state.currentTaskId=accepted.task_id;state.currentAssistantId=accepted.assistant_message_id;state.currentConversationId=accepted.conversation_id;state.currentStreamUrl=accepted.agent_stream_url||accepted.stream_url||null;state.streamLastEventId="0-0";state.terminalReceived=false;state.appliedContentEventIds.clear();state.traceEvents=[];
...
function applyFinalContentDelta(eventType,data,eventId="",assistantId=null){if(!isFinalContentDelta(eventType,data))return false;const content=String(data?.content||"");if(!content)return true;const key=contentEventIdentity(eventType,data,eventId);if(state.appliedContentEventIds.has(key))return true;state.appliedContentEventIds.add(key);const assistant=messageById(assistantId||state.currentAssistantId);if(assistant){assistant.content=String(assistant.content||"")+content;assistant.status="STREAMING";renderMessages();}return true;}
function taskStreamUrl(taskId){let raw=state.currentStreamUrl&&String(state.currentTaskId)===String(taskId)?state.currentStreamUrl:`/chat/v1/tasks/${taskId}/events`;const url=new URL(appendUserToken(raw));url.searchParams.set("final_delta_event","agent.output.delta");return url.toString();}
async function handleStreamEvent(parsed){
  state.lastStreamEventAt=Date.now();if(parsed.id)state.streamLastEventId=parsed.id;if(["heartbeat","sse.connected","sse.warning","sse.retry"].includes(parsed.event)){if(parsed.event==="sse.warning")showStreamNotice(parsed.data.message||"事件流正在重试",true);return;}
  ...
  if(!finalDelta)addExecutionEvent(row,state.currentAssistantId);
...
async function syncTaskExecutionEvents(taskId,assistantId){try{const result=await apiRequest(`/chat/v1/tasks/${taskId}/execution-events`);const rows=result?.data?.events||[];state.persistenceSyncCount+=1;for(const row of rows){try{advanceStreamCursorFromPersistedRow(row);const payload=eventPayload(row);if(isFinalContentEventRow(row)){applyFinalContentDelta(row.event_type,payload,`${row.sequence_no||row.id||""}-0`,assistantId);}else{addExecutionEvent(row,assistantId);}if(row.event_type==="entity.selection.required"){showEntitySelection({...payload,task_id:row.task_id||taskId,last_event_id:`${row.sequence_no||0}-0`});const assistant=messageById(assistantId);if(assistant)assistant.status="WAITING_SELECTION";}else if(row.event_type==="entity.selection.completed"){closeModal("entityModal");state.entityCandidates=[];state.entitySelectionTaskId=null;state.entitySelectionExpiresAt=null;}if(TERMINAL_EVENTS.has(String(row.event_type||""))){state.terminalReceived=true;const assistant=messageById(assistantId);if(assistant)assistant.status=String(payload?.status||result?.data?.task?.status||assistant.status||"COMPLETED");}}catch(error){console.error("persisted_event_apply_failed",{row,error});}}scheduleExecutionUiRefresh();return result?.data?.task||null;}catch(error){console.debug("execution_event_sync_failed",error);return null;}}
async function taskPersistenceReconciler(taskId,assistantId){while(state.isGenerating&&String(state.currentTaskId)===String(taskId)){await new Promise(resolve=>setTimeout(resolve,1000));if(!state.isGenerating)break;const task=await syncTaskExecutionEvents(taskId,assistantId)||await taskView(taskId).catch(()=>null);if(!task)continue;if(task.status==="WAITING_SELECTION"){await recoverEntitySelection(taskId);continue;}if(state.terminalReceived||TERMINAL_STATUSES.has(task.status)){state.terminalReceived=true;const assistant=messageById(assistantId);if(assistant)assistant.status=task.status;state.streamAbort?.abort();scheduleExecutionUiRefresh();break;}}}
...
async function streamTask(taskId,assistantId,resetLast=false){
  ...
  let reconnects=0;const persistenceReconciler=taskPersistenceReconciler(taskId,assistantId);
  try{
    while(!state.terminalReceived&&reconnects<8){
      try{
        ...
        if(state.terminalReceived)break;
  ...
  }finally{state.isGenerating=false;state.streamAbort=null;updateGeneratingUi();if(!state.terminalReceived)showStreamNotice(`实时事件流未正常结束，持久化通道已自动补齐到 ${state.streamLastEventId}`,true);else hideStreamNotice();await persistenceReconciler.catch(()=>{});await syncTaskExecutionEvents(taskId,assistantId);await loadConversationAfterTask();}
}
